...
class device(void_p):
    ...
    def get_device_version(self):     return GetDeviceInfo(self, DEVICE_INFO.VERSION)
    def get_global_mem_size(self):    return GetDeviceInfo(self, DEVICE_INFO.GLOBAL_MEM_SIZE)
    def get_max_mem_alloc_size(self): return GetDeviceInfo(self, DEVICE_INFO.MAX_MEM_ALLOC_SIZE)
    def get_max_work_group_size(self): return GetDeviceInfo(self, DEVICE_INFO.MAX_WORK_GROUP_SIZE)
    def get_name(self):         return GetDeviceInfo(self, DEVICE_INFO.NAME)
...
def GetDeviceIDs(platform, DEVICE_TYPE = DEVICE_TYPE.ALL) -> List[device]:
    ...
    if n > 0:
        ...
        return tuple(dev for dev in device_array)
    else:
        return ()
...
def GetDeviceInfo(device, param_name):
    if param_name == DEVICE_INFO.MAX_MEM_ALLOC_SIZE or  \
       param_name == DEVICE_INFO.GLOBAL_MEM_SIZE or \
       param_name == DEVICE_INFO.MAX_WORK_GROUP_SIZE:
...
def get_devices():
    ...
    if devices is None:
        ...
        for platform in GetPlatforms():
            for device in  GetDeviceIDs(platform, DEVICE_TYPE=(DEVICE_TYPE.GPU | DEVICE_TYPE.ACCELERATOR | DEVICE_TYPE.CPU) ):
                devices.append (device)

    return devices

def get_device_by_index(idx):
    global devices
