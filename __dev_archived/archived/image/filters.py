...
# def assert_format( img : np.ndarray, ndim=None, ch=None, dtype=None):
    
    
#     if ndim is not None:
        
        
#         if img.ndim not in ndim:
#             raise ValueError(f'img.ndim {img.ndim} not in {ndim}')
#     if ch is not None:
#         if img.ndim in [3,4]:
#             if img.shape[-1] not in ch:
#                 raise ValueError(f'img ch {img.shape[-1]} not in {ch}')    
