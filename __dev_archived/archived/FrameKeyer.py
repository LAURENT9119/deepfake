...
class FrameKeyerWorker(BackendWorker):
    ...
    def on_start(self, weak_heap : BackendWeakHeap, reemit_frame_signal : BackendSignal, bc_in : BackendConnection, bc_out : BackendConnection):
        ...
        cs.device.enable()
        cs.device.set_choices(onnx_models.RVM.get_available_devices(), none_choice_name='@misc.menu_select')
    ...
    def on_cs_devices(self, idx, device):
        ...
        if device is not None and state.device == device:
            self.rvm = onnx_models.RVM(state.device)
        else:
            state.device = device
            self.save_state()
            self.restart()
            ...
            if bcd is not None:
                ...
                if frame_image is not None:
                    ...
                    if rvm is not None:
                        ...
                        #frame_image_ip.to_ufloat32()
                        
                        alpha = rvm.extract(frame_image)[0,:,:,None]
                        
                        frame_image = frame_image.astype(np.float32)
                        
                        bg = np.empty_like(frame_image)
                        bg[:,:] = [0,255,0]
                        
                        frame_image = ne.evaluate('frame_image*alpha + bg*(1.0-alpha)')
